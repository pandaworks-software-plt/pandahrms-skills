...
Discipline backstop for ad-hoc bug investigations -- no new feature is being designed, just an existing one diagnosed.

**Core principle:** find root cause before any fix. A symptom fix is a failed debug.
...
If Phase 1 is not complete, no Edit, Write, or refactor tool call is allowed. Diagnostic instrumentation (logging, dump statements, console output) is allowed and encouraged in Phase 1.
...
- Time pressure makes "just one quick fix" tempting
- Two or more fix attempts have already failed
- You don't fully understand the failure
- Bug spans both backend (Performance/Recruitment API) and frontend (Pandahrms-Performance)
...
- User is narrating a CI failure they will fix themselves -> respond conversationally
- User identified the line and wants the edit -> apply the edit; don't run a 4-phase investigation against a 1-line change
...
- Capture the exact error string verbatim -- often searchable
- Distinguish "code threw" from "test assertion failed" -- different categories of evidence

### 2. Reproduce consistently

- Can you trigger the bug reliably? Document the exact steps.
- If intermittent, gather more data before guessing -- intermittent failures usually reveal timing, ordering, or shared-state bugs that random fixes miss.
...
- `git diff <last-known-good>..HEAD` -- if user can name a known-good ref
...
Symptom location is often where the value is *consumed*, not where it is *produced*. Fixing at consumption masks the bug for the next caller.

### 5. Multi-component evidence (FE <-> BE, CI <-> build, API <-> service <-> DB)

When the system has multiple components and the failure could plausibly live in any of them, do NOT guess which one is broken. Add diagnostic instrumentation at every boundary, run once, and let evidence point at the failing component.
...
2. **Compare against the working example.** Read it completely -- not just parts that look relevant. Patterns break in places you didn't think mattered.

3. **List every difference.** Even ones you think "can't matter." Differences in attribute order, casing, DI registration, package version, namespace, file location have all caused real Pandahrms bugs.
...
2. **Test minimally.** Smallest possible change that proves or disproves the hypothesis. One variable at a time. If you change two things and it works, you don't know which one was the fix.
...
- Pandahrms-Performance frontend: `pnpm vitest run <path>` for unit/component tests. For UI-level repro, drive the failing flow through Playwright MCP (`mcp__playwright__*` tools) -- no project-level `playwright test` CLI to invoke here.
- If the bug is in code with no test infrastructure path, surface that to the user before continuing -- don't silently skip the test.

The failing test is proof you understood the bug. Without it, "the fix worked" only proves the fix changed *something*.
...
- If you notice a separate issue in the same file, surface it as a follow-up rather than fixing inline.

### 3. Verify

- Failing test from step 1 now passes.
- No previously passing tests are now failing.
- Original symptom (user's report) is actually gone -- not just the test green.
...
When each fix reveals a new problem in a different place, or each requires "massive refactoring" to land, the pattern is wrong, not the implementation.

Surface to the user with concrete observations:
...
2. Confirm the working tree contains: the fix, the failing test (now green), nothing else.
3. Tell the user the bug is resolved and the next step is `/athena-code-review` (which then routes to `/hermes-commit`).

This skill does NOT invoke `/athena-code-review` or `/hermes-commit` directly. User controls when review and commit happen.
...
| "Multiple fixes at once saves time" | Can't isolate which one worked. Causes new bugs. |
...
| "One more fix attempt" (after 2+ failures) | 3+ failures means architecture is wrong, not the next fix. |
...
- Run `/athena-code-review` automatically -- user invokes it after the fix lands
- Invoke `atlas-pipeline-orchestrator` -- atlas is for new work; this skill is for existing-bug diagnosis
- Design refactors -- when 3+ fixes fail, escalate to user, who chooses whether to start an atlas pipeline
- Write new specs or update Gherkin features -- if a bug fix changes documented behavior, surface as follow-up; spec updates go through `/spec-writing`
- Run database migrations, restore .bak files, or modify local Docker MSSQL state outside of read-only diagnostic queries
...
| Fixing at symptom site, not source | Trace data flow backwards (Phase 1 step 4). |
| Layering a second fix on top of an unconfirmed first guess | One hypothesis at a time. Disprove before pivoting. |
| Skipping the failing test "because the fix is obvious" | Failing test is proof of understanding. Always write it. |
