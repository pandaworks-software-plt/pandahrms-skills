...
<!-- Auto-generated by development-workflow. Do not edit. -->
| Phase | Duration |
|-------|----------|
...
3. **Track time per task** -- Follow the Time Tracking instructions above. Record start/end timestamps for every plan task. On completion, read the `## Pipeline Timing` section from the plan file and display the combined Development Summary with grand total.
