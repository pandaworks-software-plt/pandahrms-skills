...
Write/update the **L1 behaviour spec** in `pandahrms-spec` from intake output. L1 = tech-agnostic product truth (the WHAT + when done), Gherkin, no UI mechanics, no API/endpoint detail, no field-level form steps. Write L1 only -- never L2 FE/BE executable specs.
