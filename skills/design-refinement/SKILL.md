...
1. **Locate `pandahrms-spec`** -- it is typically a sibling directory of the working project (e.g. `~/Developer/pandaworks/_pandahrms/pandahrms-spec`). Use `git rev-parse --show-toplevel` from the working project, then look for `pandahrms-spec` as a sibling under the parent. If not found, ask inline in plain text, listing these options for the user to type back: 'Path is <X>' (user types path), 'No pandahrms-spec in this workspace -- skip alignment and treat as non-Pandahrms', 'Stop -- I will set this up first'. Do NOT proceed past step 6 until one of those options is chosen.
